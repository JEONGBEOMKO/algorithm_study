...
public class 스타트와링크001 {
	
	static int n; //사람 수 입력 변수 
	static int map[][],check[];  //능력치 판 2차원 배열, 각 사람 수의 방문 배
	static int min = Integer.MAX_VALUE; //최속값을 찾기 위한 변수
	
	//DFS 메소드, 조합의 개념을 사용하므로 별도의 시작 부분을 전달해 주어야 한다.
	
	static void dfs(int start, int cnt) {
		if(cnt == n/2) { // DFS 종료 후 , n/2의 사람 수 만큼 선택하면 계산을 시작한다.
			cal(); //최솟값을 계산하는 메소드 
